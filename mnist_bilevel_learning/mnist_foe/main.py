...
def print_progress(epoch, batch, total_batches, loss, theta, info=""):
    ...
    sys.stdout.write(f"\r[{info}] Ep {epoch+1} | Batch {batch+1}/{total_batches} | "
                     f"Loss: {loss:.4f} | GW: {gw:.3f} | FW: {mean_fw:.4f} | nu: {mean_nu:.4f}")
    sys.stdout.flush()
...
def run_experiment():
    ...
    print(f"Starting MNIST Experiment (HOAG + FoE Regularizer)")
    print(f"FoE: {NUM_EXPERTS} experts, {FILTER_SIZE}x{FILTER_SIZE} filters, "
          f"{THETA_SIZE} total params")
    ...
    results = {}
    dummy_theta = torch.zeros(THETA_SIZE, device=Config.DEVICE)

    # Load pre-trained model
    ckpt_path = Config.UPPER_BOUND_CKPT
    print(f"\nLoading pre-trained classifier from: {ckpt_path}")
    ...
    # PHASE 3: Approach 1 (HOAG + FoE - Theta Only)
    print("\n--- PHASE 3: Approach 1 (HOAG + FoE - Theta Only) ---")
    ...
    sched_theta = torch.optim.lr_scheduler.CosineAnnealingLR(opt_theta, T_max=Config.EPOCHS_HOAG, eta_min=1e-4)
    ...
    print("\n--- PHASE 4: Approach 2 (Joint Learning - FoE Theta + Classifier) ---")
    ...
    theta = initialize_theta(Config.DEVICE).requires_grad_(True)
    opt_theta = torch.optim.Adam([theta], lr=Config.LR_THETA)
    sched_theta_joint = torch.optim.lr_scheduler.CosineAnnealingLR(opt_theta, T_max=Config.EPOCHS_JOINT, eta_min=1e-4)
    ...
    with open(results_path, "w") as f:
        f.write("=== FINAL RESULTS (MNIST + FoE Regularizer) ===\n")
        f.write(f"Blur sigma: {Config.BLUR_SIGMA} | Noise: {Config.NOISE_SIGMA}\n")
        f.write(f"FoE: {NUM_EXPERTS} experts, {FILTER_SIZE}x{FILTER_SIZE} filters, "
                f"{THETA_SIZE} params\n")

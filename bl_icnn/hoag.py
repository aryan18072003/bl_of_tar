...
def solve_inner_problem(w_init, theta, y, physics_op, inner_loss_fn,
                        state, icnn, sfb, l2_net,
                        lr=0.02, max_steps=100, verbose=0):
    ...
    with torch.enable_grad():
        loss_init = inner_loss_fn(w, theta.detach(), y, physics_op, icnn, sfb, l2_net)
    ...
    for step in range(max_steps):
        optimizer.zero_grad()
        loss = inner_loss_fn(w, theta.detach(), y, physics_op, icnn, sfb, l2_net)
        loss.backward()
...
def hoag_step(theta, y, physics_op, model, loss_fn, mask,
              inner_loss_fn, state, icnn, sfb, l2_net,
              inner_lr=0.02, inner_steps=100,
    ...
    w_star, n_inner = solve_inner_problem(
        ...
        state, icnn, sfb, l2_net,
        lr=inner_lr, max_steps=inner_steps, verbose=verbose
    )
    
    if verbose > 0:
        inner_loss_val = inner_loss_fn(w_star.detach(), theta.detach(), y, physics_op, icnn, sfb, l2_net)
    ...
    q = conjugate_gradient(
        ...
        b=val_loss_grad_w,
        icnn=icnn, sfb=sfb, l2_net=l2_net,
    ...
    # --------------------------------------------------
    # CROSS-DERIVATIVE: ∇²_wθ h(w*, θ) · q
    #
    # Since h = ||y-A(w)||² + R_θ(w), and the data fidelity term
    # does NOT depend on θ, we only need ∇²_wθ R_θ(w) · q.
    # This avoids differentiating through grid_sample.
    # --------------------------------------------------
    ...
    with torch.enable_grad():
        reg_cross = regularizer_only(w_for_cross, theta_for_cross, icnn, sfb, l2_net)
        grad_w_reg = autograd.grad(reg_cross, w_for_cross, create_graph=True)[0]
        grad_dot_q = torch.sum(grad_w_reg * q.detach())
